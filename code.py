...
def markAttendance(regNumber, name):
    f = open("attendance_using_face_recognition\\Attendance.csv", "r+")
    data = f.readlines()
    regNumberList = []
    for line in data:
        entry = line.split(',')
...
loc="attendance_using_face_recognition\\imageData"
allPaths = os.listdir("attendance_using_face_recognition\\imageData")
allNames = []
allRegNumbers = []
allEncodings = []
for index in range(len(allPaths)):
    ...
    image = face_recognition.load_image_file(join(loc,allPaths[index]))
...
while True:
    ...
    for encoding in faceEncoding:
        ...
        if ismatched[bestMatchIndex] and accuracy > 70:
